...
export default function PhotosUploader({pic,onChange}){
    ...
    async function addPicLink(ev){
      ...
      function selectAsMainPhoto(ev,filename){
         ev.preventDefault();
         onChange([filename,...pic.filter(photo => photo !== filename)]);
         // onChange(newPic);
      }
    return(
        <>
                   ...
                   <div className="mt-2 grid gap-2 grid-cols-3  md:grid-cols-4 lg:grid-cols-6">
                      {pic.length > 0 && pic.map(link => (
                        ...
                        {/* {link} */}
                           ...
                           <button onClick={(ev)=>selectAsMainPhoto(ev,link)} className="cursor-pointer absolute bottom-1 left-1 text-white bg-black  bg-opacity-50 rounded-2xl py-2 px-2">
                           {link === pic[0] && (
